...
export const SubmissionResultPage: React.FC = () => {
  ...
  const hints = getHintsForProblem(slug || '');
  const resultRows = results || [];
  const sampleCases = problem?.sample_test_cases ?? [];
  const sampleCaseById = new Map(sampleCases.map(tc => [tc.id, tc]));
  const sampleResults = resultRows.filter(r => r.is_sample || sampleCaseById.has(r.test_case_id));
  const hiddenResults = resultRows.filter(r => !r.is_sample && !sampleCaseById.has(r.test_case_id));
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className={`relative overflow-hidden rounded-2xl border ${statusBg} p-6 md:p-8 transition-all duration-300`}>
        ...
        {submission.error_message && (
          <div className="mt-6 bg-rose-500/5 border border-rose-500/10 rounded-xl p-4 font-mono text-xs text-rose-400 whitespace-pre-wrap break-words">
            <span className="font-black uppercase tracking-wider text-rose-500 block mb-1">Diagnostic Log:</span>
            {submission.error_message}
          </div>
        )}
      </div>
      ...
      {showResultBreakdown && (
        <div className="bg-dark-panel border border-dark-border rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="p-4">
            {isLoadingResults ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {resultRows.map((r) => {
                  const sampleCase = sampleCaseById.get(r.test_case_id);
                  const sampleIndex = sampleCases.findIndex(tc => tc.id === r.test_case_id);
                  const hiddenIndex = hiddenResults.findIndex(hr => hr.id === r.id);
                  const caseLabel = sampleCase || r.is_sample
                    ? `Sample ${sampleIndex >= 0 ? sampleIndex + 1 : sampleResults.findIndex(sr => sr.id === r.id) + 1}`
                    : `Hidden ${hiddenIndex + 1}`;
                  const input = r.test_case_input ?? sampleCase?.input ?? null;
                  const expected = r.expected_output ?? sampleCase?.expected_output ?? null;
                  const stdout = r.stdout?.trim() || null;
                  const hasDetails = input !== null || expected !== null || stdout !== null || Boolean(r.stderr);
                    ...
                    >
                      <div className="flex items-center justify-between gap-3 select-none">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="font-bold text-gray-200 truncate">{caseLabel}</span>
                          {!sampleCase && !r.is_sample && (
                            <span className="text-[9px] font-bold uppercase tracking-wider text-gray-500 bg-dark-bg border border-dark-border px-1.5 py-0.5 rounded">
                              Hidden
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-mono text-gray-500 shrink-0">
                          ...
                          <span>{r.memory_kb} KB</span>
                        </div>
                      </div>

                      {hasDetails && (
                        <div className="mt-3 grid grid-cols-1 gap-2 font-mono">
                          {input !== null && (
                            <pre className="bg-dark-bg/70 border border-dark-border rounded-lg p-2.5 text-gray-300 overflow-x-auto max-h-28">
                              <span className="text-gray-500">Input: </span>{input}
                            </pre>
                          )}
                          {expected !== null && (
                            <pre className="bg-dark-bg/70 border border-dark-border rounded-lg p-2.5 text-gray-300 overflow-x-auto max-h-28">
                              <span className="text-gray-500">Expected: </span>{expected}
                            </pre>
                          )}
                          {stdout !== null && (
                            <pre className={`border rounded-lg p-2.5 overflow-x-auto max-h-28 ${r.passed ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-300' : 'bg-rose-500/5 border-rose-500/20 text-rose-300'}`}>
                              <span className="text-gray-500">Output: </span>{stdout}
                            </pre>
                          )}
                          {r.stderr && (
                            <pre className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-2.5 text-amber-200 overflow-x-auto max-h-28">
                              {r.stderr}
                            </pre>
                          )}
                        </div>
      ...
      {/* Grid with Interactive Hints and Detail Breakdown */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* Left column - Hints */}
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-dark-panel border border-dark-border rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex items-center gap-2 select-none border-b border-dark-border pb-3">
              <div className="p-1 bg-amber-500/10 rounded-lg text-amber-500">
                <Lightbulb className="w-4 h-4" />
              </div>
              <h2 className="text-sm font-bold text-gray-200">Interactive Hints</h2>
            </div>
            
            <p className="text-xs text-gray-500 leading-relaxed select-none">
              Need help optimizing your solution or diagnosing issues? Click each step to reveal detailed hints one by one.
            </p>
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className={`w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-mono font-bold ${
                          isRevealed ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-dark-bg text-gray-500'
                        }`}>
                          {idx + 1}
                        </span>
                        Hint {idx + 1}
          ...
          {/* Execution details limit cards */}
          <div className="bg-dark-panel border border-dark-border rounded-xl p-5 shadow-sm space-y-4 select-none">

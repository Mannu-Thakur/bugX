    ...
    <div className="flex gap-2.5 mb-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        {message.error && !message.isStreaming && (
          <div className="mt-1.5 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-200/90 text-xs flex items-center justify-between gap-3 shadow-sm backdrop-blur-sm">
            <div className="flex items-center gap-2 font-medium min-w-0">
              <span className="text-amber-400 text-xs shrink-0">⚠️</span>
              <span className="truncate text-[11px] font-semibold">{message.error}</span>
            </div>
            {onRegenerate && !message.content && (
              <button
                onClick={onRegenerate}
                className="px-2 py-0.5 rounded-md bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 hover:text-white transition-colors cursor-pointer text-[10px] font-bold shrink-0 border border-amber-500/30"
              >
                Retry
              </button>
            )}

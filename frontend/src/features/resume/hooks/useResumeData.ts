/**
 * Resume data hook — fetches from existing APIs only.
...
async function resumeRequest<T>(path: string): Promise<T> {
...
export function useResumeData() {
  const stats = useQuery<UserStats>({
    ...
    queryFn: () => resumeRequest<UserStats>('/users/me/stats'),
    staleTime: 5_000,
    refetchOnWindowFocus: true,
  });

  const submissions = useQuery<{ items: SubmissionSummary[] }>({
    queryKey: ['resume-submissions'],
    queryFn: async () => {
      const data = await resumeRequest<{ items: SubmissionSummary[] }>('/users/me/submissions?page=1&limit=100');
